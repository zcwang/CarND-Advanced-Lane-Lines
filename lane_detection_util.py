...
def undistort(image, dist_matrix, dist_param):
    undis_image = cv2.undistort(image, dist_matrix,dist_param, None, dist_matrix)
    return undis_image


def preprocess(image, mask_vertices, color_thresh, gray_thresh):
...
def select_lane_lines(warped_lane, sliding_window=[80,120]):
    ...
    for i in range(6):
        ...
        left_lane_windowed = warped_lane[current_height_window[0]:current_height_window[1],
                                         current_left_window[0]:current_left_window[1]]
        right_lane_windowed = warped_lane[current_height_window[0]:current_height_window[1],
...
def cal_curvature(lr_binary_images):
    y_eval = np.max(lr_binary_images)
...
def print_info(image, radius, deviation):
    img = np.copy(image)
    cv2.putText(img, "Curve Radius:{}m".format(int(radius)), (30,30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (225,225,225), 2, cv2.LINE_AA)
    cv2.putText(img, "Center Deviation:{0:.2f}m".format(deviation), (30, 70),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (225,225,225), 2, cv2.LINE_AA)
    return img


def lane_detection(img, dist_matrix, dist_param, mask_vertices, color_thresh, gray_thresh, src, dst, sliding_window):
    image = np.copy(img)
    undist_image = undistort(image,dist_matrix=dist_matrix,dist_param=dist_param)
    warped_lane = perspective_transform(preprocess(undist_image,mask_vertices,color_thresh,gray_thresh),src,dst)
